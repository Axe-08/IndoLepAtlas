...
SPECIES_LIST_LOG = "species_list.log"  # Cached species URLs
...
def get_species_links(limit=None):
    """
    Returns species links. Loads from local cache (species_list.log) if available,
    otherwise scrapes the site and saves the result to cache.
    ...
    if os.path.exists(SPECIES_LIST_LOG):
        ...
        if links:
            logging.info(f"Loaded {len(links)} species URLs from local cache ({SPECIES_LIST_LOG}).")
    ...
    # --- Save full list to cache (always save without limit) ---
    with open(SPECIES_LIST_LOG, "w") as f:
        for url in species_links:
            f.write(url + "\n")
    logging.info(f"Scraped and cached {len(species_links)} species URLs to {SPECIES_LIST_LOG}.")
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if log_file == SPECIES_LIST_LOG:
                    # For species list: remote is authoritative if we have nothing local.
                    # Merge both and preserve order (remote first, then any local extras).
                    merged = list(dict.fromkeys(remote_lines + list(existing)))  # dedupe, preserve order
                    with open(log_file, "w") as f:
                        for line in merged:
                            if line.strip():
                                f.write(line.strip() + "\n")
                    logging.info(f"Merged remote {log_file} ({len(remote_lines)} lines). Total now: {len(merged)}")
                else:
...
def run_crawler(chunk=1, total_chunks=1):
    ...
    # --- Phase 1: Pull HF Logs (incl. species list cache) & Re-upload leftover batches ---
    reupload_count = 0
    reupload_files = 0
    if hf_ready:
        pull_logs_from_hf()
    ...
    # --- Phase 3: Get species links (from cache or scrape) ---
