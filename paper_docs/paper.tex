...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{Systematic baseline establishment:} An 11-configuration ablation study across loss dynamics, architectural feature fusion, and transfer-learning strategies, providing reproducible baselines for future work on IndoLepAtlas.
    ...
    %\item \textbf{Geotemporal fusion:} First integration of biogeographic zone and seasonal embeddings into a butterfly classification pipeline. % UNCOMMENT AFTER PHASE 5 RESULTS
\end{enumerate}
...
IndoLepAtlas comprises 61,344 images spanning 966 butterfly species and 127 larval host plant species, curated from the Indian Lepidoptera web atlas and iNaturalist with rigorous quality control. The filtering pipeline removes non-butterfly entries, resolves taxonomic synonyms, and separates early life-stage images from adult specimens (60,641 images) using both metadata tags and filename pattern matching. Adult images are used for classification training; early-stage images are retained as labelled auxiliary data for future life-stage detection and species-lifecycle modelling.

\subsection{Taxonomic and Geographic Coverage}
The dataset covers six butterfly families: Nymphalidae (7,168), Lycaenidae (6,769), Hesperiidae (6,416), Pieridae (1,821), Papilionidae (1,555), and Riodinidae (417)--representing approximately 64\% of India's known butterfly diversity. Geographic coverage spans 34 Indian states mapped to 9 biogeographic zones, with primary contributions from Maharashtra (4,383), Karnataka (3,065), and Arunachal Pradesh (2,918). Metadata coverage: 98.3\% location, 76.1\% date, 85.8\% sex missing (acknowledged limitation). Temporal distribution spans all 12 months with a monsoon peak (August--November), reflecting natural butterfly activity patterns.
...
Class distribution ranges from 543 images (\emph{Athyma selenophora}) to $<$5 images (tail species). Classes are stratified into \textbf{Dense} ($\geq$50 images) and \textbf{Sparse} ($<$50 images) strata. Data is split into train (43.5\%), validation (22.7\%), and test (33.8\%) partitions.

% === iNaturalist Coverage Comparison (to be filled after inat_coverage.py completes) ===
% \subsection{Coverage Gap Analysis}
% Cross-referencing IndoLepAtlas species against iNaturalist India research-grade observations reveals
% that X\% of our 961 species have fewer than 100 India-specific iNat records, and Y species have
% zero records. This confirms that global repositories severely under-represent India-specific
