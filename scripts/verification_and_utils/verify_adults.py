...
def visualize_samples(json_path, image_dir, output_dir, num_samples=10):
    ...
    count = 0
    # Sort or shuffle if needed, but let\s just pick the first few non-larvae
    for img_info in coco["images"]:
        if "EarlyStage" in img_info["file_name"]:
            continue
            ...
            if img_id in ann_map:
                for ann in ann_map[img_id]:
                    ...
                    if len(bbox) == 4:
                        ...
                        ax.add_patch(rect)
                        ax.text(x, max(y - 5, 0), f"{cat_name}", color="cyan", fontsize=8, 
                                bbox=dict(facecolor="black", alpha=0.5))

            plt.axis("off")
            fname = os.path.basename(img_path)
            out_path = os.path.join(output_dir, f"adult_box_{fname}")
...
if __name__ == "__main__":
    visualize_samples(
        ...
        output_dir="butterfly_adult_viz",
        num_samples=8
    )
